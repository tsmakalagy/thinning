...
public:
  ...
  inline Mat1b first_img() {
    return _first_img.clone();
  }

  Mat1b current_skel() const {
    return _thinner.get_skeleton().clone();
  }

  inline Mat1b contour_brighter(const Mat1b & img) {
  ...
  inline Mat3b contour_color(const Mat1b & img) {
    _contour_viz.from_image_C4(img);
...
void generate_video_bw(const Mat1b & query,
  ...
  Size output_size(it.cols() * 2, it.rows() * 2);
  ostringstream out_filename;
  out_filename << implementation_name << "_bw.avi";
  VideoWriter writer(out_filename.str(), codec, 20,
                         output_size, false); // BW
  assert(writer.isOpened());
  Mat1b curr_resized;
  resize((show_contour_brighter ? it.contour_brighter(it.first_img()) : it.first_img()),
             curr_resized, output_size, CV_INTER_NN);
  writer << curr_resized;
  imshow("curr_resized", curr_resized); waitKey(10);

  // loop
  while (!it.has_converged()) {
    it.iter();
    resize((show_contour_brighter ? it.contour_brighter(it.current_skel()) : it.current_skel()),
               curr_resized, output_size, CV_INTER_NN);
    writer << curr_resized;
    imshow("curr_resized", curr_resized); waitKey(10);
  }
...
template<class T>
void paste_images_gallery(const vector<Mat_<T> > & in,
                Mat_<T> & out,
                int gallerycols, T background_color,
                bool draw_borders = false, Scalar border_color = CV_RGB(0,0,0)) {
  if (in.size() == 0) {
    out.create(0, 0);
    return;
  }
  ...
  // prepare output
  int galleryrows = ceil(1. * nimgs / gallerycols);
  ...
  for (int img_idx = 0; img_idx < nimgs; ++img_idx) {
    ...
    Rect roi(galleryx * cols1, galleryy * rows1, cols1, rows1);
    ...
    if (cols1 != in[img_idx].cols || rows1 != in[img_idx].rows) {
      ...
      line(out, roi.tl(), roi.br(), border_color, 2);
      line(out, roi.br(), roi.tl(), border_color, 2);
    }
    else
      in[img_idx].copyTo( out(roi) );
    if (draw_borders)
      rectangle(out, roi, border_color, 1);
  } // end loop img_idx
} // end paste_images_gallery<_T>

////////////////////////////////////////////////////////////////////////////////

void generate_video_comparer(const Mat1b & query,
                             bool crop_img_before,
                             int gallerycols,
                             const string out_prefix = "comparer_") {
  // init iterators
  static const unsigned int nimpls = 3;
  string impls[] = {IMPL_MORPH, IMPL_ZHANG_SUEN_FAST, IMPL_GUO_HALL_FAST};
  Mat3b curr, curr_resized;
  ...
  Mat3b first_img_color;
  // first_img_color = its[0].contour_color(its[0].first_img());
  cvtColor(its[0].first_img(), first_img_color, CV_GRAY2BGR);
  int /*cols1 = first_img_color.cols, */ rows1 = first_img_color.rows;
  //Point text_pos(cols1 / 2, rows1 - 10);
  // image_utils::draw_text_centered
  Point text_pos(10, rows1 - 10);
  putText(first_img_color, "query",
              text_pos, CV_FONT_HERSHEY_PLAIN, 1, CV_RGB(0, 255, 0));

  // video writer
  // for a weird reason, odd width and height result in an empty video
  Size output_size
      (gallerycols * 2 * its[0].cols(), ceil((double)(4 / gallerycols) * 2 * its[0].rows()));
  unsigned int fps = 5;
  ostringstream out_filename;
  out_filename << out_prefix << ".avi";
  VideoWriter writer(out_filename.str(), codec, fps, output_size, true); // color
  assert(writer.isOpened());
  ...
  while(n_terminated < nimpls) {
    ...
    for (unsigned int impl_idx = 0; impl_idx < nimpls; ++impl_idx) {
      if (its[impl_idx].has_converged())
        ...
        its[impl_idx].iter();
      Mat3b curr_img_color = its[impl_idx].contour_color(its[impl_idx].current_skel());
      dilate(curr_img_color, curr_img_color, Mat());
      //image_utils::draw_text_centered
      putText
    ...
    // now make collage
    paste_images_gallery(curr_imgs, curr, gallerycols, Vec3b(0, 0, 0), true, CV_RGB(255, 255, 255));
    resize(curr, curr_resized, output_size, CV_INTER_NN);
    writer << curr_resized;
    imshow("curr", curr); waitKey(10);
    ...
    ostringstream name; name << "comparer_" << setprecision(3) << setfill('0') << iters << ".png";
    imwrite(name.str(), curr);
  } // end while1
  ...
  ostringstream out_filename_lossless;
...
void benchmark(const Mat1b & query,
               bool display_imgs = true) {
  VoronoiThinner thinner;
  unsigned int ntimes = 10;
  Timer timer;
  vector<string> impls = VoronoiThinner::all_implementations();
  vector<Mat1b> skels_no_crop;
  vector<Mat1b> skels_crop;
  ...
  for (unsigned int i = 0; i < impls.size(); ++i) {
    imshow(impls[i] + string("nocrop"), skels_no_crop[i]);
    imshow(impls[i] + string("crop"), skels_crop[i]);
  } // end loop i
  waitKey(0);
} // emd benchmark();
...
void benchmark_logs(const Mat1b & query) {
  unsigned int npixels_curr = query.cols * query.rows;
  VoronoiThinner thinner;
  vector<string> implementation_names;
  ...
  for (unsigned int imp_idx = 0; imp_idx < implementation_names.size(); ++imp_idx)
    cout << implementation_names[imp_idx] << " \t";
  cout << endl;
  ...
  for (unsigned int npixels = 100; npixels <= 1E7; npixels*=10) {
    ...
    resize(query, query_resized, Size(), ratio, ratio, CV_INTER_NN);
    ...
    for (unsigned int imp_idx = 0; imp_idx < implementation_names.size(); ++imp_idx) {
      ...
      cout << timer.getTimeMilliseconds() / ntimes << " \t";
    } // end loop imp_idx
    cout << endl;
  } // end loop npixels
} // end benchmark_logs();
...
int CLI(int argc, char** argv) {
  ...
  if (order == THIN) {
    ...
    for (unsigned int file_idx = 0; file_idx < files.size(); ++file_idx) {
      ostringstream out; out << "out_" << file_idx;
...
int main(int argc, char** argv) {
  // benchmark_logs(imread(IMG_DIR "depth/juggling1_user_mask.png", CV_LOAD_IMAGE_GRAYSCALE));

  //  generate_video_bw(imread(IMG_DIR "depth/juggling1_user_mask.png", CV_LOAD_IMAGE_GRAYSCALE),
  ...
  //  generate_video_comparer(imread(IMG_DIR "skeletons/horse.png", CV_LOAD_IMAGE_GRAYSCALE),
  //                          true, 2, "horse_");
  //  generate_video_comparer(imread(IMG_DIR "depth/juggling1_user_mask.png", CV_LOAD_IMAGE_GRAYSCALE),
  //                          true, 4, "comparer_juggling1_mask_");
  //  generate_video_comparer(imread(IMG_DIR "depth/alberto1_user_mask.png", CV_LOAD_IMAGE_GRAYSCALE),
  //                          true, 4, "alberto1_mask_");
  //  generate_video_comparer(imread(IMG_DIR "skeletons/japanese_src.png", CV_LOAD_IMAGE_GRAYSCALE),
  //                          true, 2, "comparer_japanese_src_");
  //  generate_video_comparer(imread(IMG_DIR "skeletons/opencv_src.png", CV_LOAD_IMAGE_GRAYSCALE),
  //                          true, 1, "opencv_src_");
  //  generate_video_comparer(imread(IMG_DIR "powerXML/power.png", CV_LOAD_IMAGE_GRAYSCALE),
